...
LUC_plot_compare_pathways_percent <- function(list_res, rasterfile, grid, ns_map, names, Path=NULL, LU=NULL, color = "Greens", label = "Area in % of pixel"){
  ...
  for(cur in 1:length(list_res)){
    ...
    temp <- list_res[[cur]]$out.res %>% 
      ...
      mutate(value = 100 * value/(area/1000)) %>% 

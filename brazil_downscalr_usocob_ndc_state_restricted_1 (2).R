...
# (UsoCobertura__fable_v2.xlsx, sheet usoecob2020) replacing hilda_br_2015.xlsx.
#
...
# =============================================================================
# Keys = exact column names from the (ha) columns in usoecob2020 (without " (ha)").
...
col_to_lu <- c(
  ...
  "Urban"          = "Urban",
  "Mining"         = "Urban",
  "PhotovoltaicPP" = "Urban"

  # Excluded: NoData, Aquaculture, ComForestry
)

# =============================================================================
# B.  Load usoecob2020 and build LU stock table
# =============================================================================

message("Reading UsoCobertura__fable_v2.xlsx | sheet: usoecob2000 ...")
raw <- read_excel("UsoCobertura__fable_v2.xlsx", sheet = "usoecob2000")
...
brazil_FABLE <- read_br("brazil_fable_ct.xlsx") %>%
...
states_shp <- sf::st_read("C:/Users/User/Desktop/FABLE/DownscalingFABLE/br_states.shp", quiet = TRUE) %>% sf::st_make_valid()
biomes     <- sf::st_read("C:/Users/User/Desktop/FABLE/DownscalingFABLE/br_biomes.shp", quiet = TRUE)
...
restrictions_br <- expand.grid(lu.from = LU_CLASSES, lu.to = c("Pasture", "Urban", "Cropland"),
...
message("Saved: results_DS_usocob.rds, downscaled_LUC_usocob.rds/csv")
...
cropland_2020 <- downscaled_LUC %>%
  filter(lu.from == lu.to, lu.to == "Forest", times == "2050") %>%
  group_by(ns) %>%
...
total_Mha <- sum(cropland_2020$value_kha, na.rm = TRUE) / 1000
message(sprintf("Cropland 2020 (CT) total = %.2f Mha", total_Mha))
...
reclass_mat <- as.matrix(cropland_2020[, c("id_c","value_kha")])
...
shp_states_path <- "C:/Users/User/Desktop/FABLE/DownscalingFABLE/br_states.shp"
shp_biomes_path <- "C:/Users/User/Desktop/FABLE/DownscalingFABLE/br_biomes.shp"
...
png("cropland_2020_ct_states_breaks.png", width = 760, height = 820, res = 105, bg = "white")
...
terra::plot(
  ...
  axes   = TRUE,
  mar    = c(3, 3, 3, 1),
  main   = sprintf("CT | Cropland | 2020 | Total: %.2f Mha", total_Mha),
  cex.main = 0.95
)
...
dev.off()
message("Saved: cropland_2020_ct_states_breaks.png")

saveRDS(cropland_2020, "cropland_2020_ct_states_kha.rds")
...
verify_tbl <- full_join(area_start, area_end, by = c("LandCoverInit", "YearEnd")) %>%
  ...
  mutate(
    YearStart      = lag(YearEnd),
    NextAreaStart  = lead(AreaStart),                       # AreaStart of the following period
  ...
  ungroup() %>%
  filter(!is.na(YearStart)) %>%                             # drop the row with no prior period
...
write.csv(verify_tbl, "area_continuity_check_usocob_states.csv", row.names = FALSE)
message("Saved: area_continuity_check_usocob_states.csv")
